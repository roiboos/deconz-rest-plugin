...
int DeRestPluginPrivate::setGroupState(const ApiRequest &req, ApiResponse &rsp)
{
    ...
    bool hasLevel = map.contains("level");
    ...
    {
	hasLevel = false;
    	QString direction = map["level"].toString();
	...
	if(direction == "up" || direction == "down" || direction == "stop")
		...
		if(addTaskMoveLevel(task, false, dirUp, rate))
		{
	                QVariantMap rspItem;
	                QVariantMap rspItemState;
	                rspItemState[QString("/groups/%1/state/level").arg(id)] = direction;
	                rspItem["success"] = rspItemState;
	                rsp.list.append(rspItem);
	...
	{
            rsp.list.append(errorToMap(ERR_INVALID_VALUE, QString("/groups/%1/state/level").arg(id), QString("invalid value, %1, for parameter, level").arg(map["level"].toString())));
            rsp.httpStatus = HttpStatusBadRequest;
            return REQ_READY_SEND;
	}
    }	
